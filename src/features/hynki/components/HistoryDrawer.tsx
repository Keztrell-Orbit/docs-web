...
export function HistoryDrawer({ isOpen, onClose, versions, onRestore }: HistoryDrawerProps) {
  ...
  return (
    <div className="bg-[#F1F0EA] p-4 animate-slide-up flex flex-col space-y-3 z-10 max-h-[300px] overflow-y-auto" id="document-history-drawer">
      ...
      <div className="space-y-1.5">
        {versions.map((v, i) => (
          <button
            key={i}
            onClick={() => onRestore(v)}
            className="w-full text-left text-xs p-2.5 rounded-none bg-white hover:bg-[#F1F0EA] transition-colors flex items-center justify-between shadow-xs"
          >
