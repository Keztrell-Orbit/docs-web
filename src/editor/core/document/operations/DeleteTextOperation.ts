...
import { InsertTextOperation } from "./InsertTextOperation.ts";

let opCounter = 0;

export class DeleteTextOperation implements DocumentOperation {
  readonly id: string;
  ...
  constructor(
    ...
    this.id = `delete-text-${Date.now()}-${opCounter++}`;
    ...
    return {
      ...
      blocks: document.blocks.map((b: Block) => {
        ...
        if (b.type === "heading") {
          ...
          return { ...b, text: text.slice(0, this.offset) + text.slice(this.offset + this.length) } as HeadingBlock;
        }
        if (b.type === "paragraph") {
          const text = (b as ParagraphBlock).text;
          return { ...b, text: text.slice(0, this.offset) + text.slice(this.offset + this.length) } as ParagraphBlock;
        }
        return b;
      }),
    };
  ...
  invert(_originalDocument: Document): DocumentOperation {
    return new InsertTextOperation(this.blockId, this.offset, this.deletedText);
  }
  ...
  validate(document: Document): ValidationError | null {
    ...
    if (block.type !== "heading" && block.type !== "paragraph") {
      return {
        ...
        message: `Block "${this.blockId}" is type "${block.type}", cannot delete text`,
        operationId: this.id,
  ...
  describe(): string {
    return `DeleteText(block="${this.blockId}", offset=${this.offset}, length=${this.length})`;
  }
}
